...
volatile sig_atomic_t keep_run = 1;

// Signal handler function that sets keep_run to 0 when SIGINT is received.
void my_sig_handler(int signum) {
    // Checks if the signal received is SIGINT.
    if (signum == SIGINT) {
        keep_run = 0; // Stops the main loop and allows the program to exit gracefully.
    }
}
...
int main() {
    ...
    // Continues to run until keep_run is set to 0 by the signal handler.
    while(keep_run) {
